...
class TelegripSystem:
    ...
    def __init__(self, config: TelegripConfig):
        ...
        self.vr_events_queue = queue.Queue(maxsize=100)
        
        # Components
        self.https_server = HTTPSServer(config)
        self.vr_server = VRWebSocketServer(self.command_queue, config, event_queue=self.vr_events_queue)
    ...
    def get_vr_events(self):
        """Drain VR button events emitted by the websocket server."""
        events = []
        try:
            while True:
    ...
    async def _soft_restart_sequence(self):
        ...
        try:
            ...
            self.vr_server = VRWebSocketServer(self.command_queue, self.config, event_queue=self.vr_events_queue)
