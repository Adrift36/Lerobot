...
class VRWebSocketServer(BaseInputProvider):
    ...
    def __init__(self, command_queue: asyncio.Queue, config: TelegripConfig, event_queue=None):
    ...
    def _emit_event(self, event_type: str, hand: str):
        if self.event_queue is None:
            return

        try:
            self.event_queue.put_nowait({"type": event_type, "hand": hand, "timestamp": time.time()})
        except queue.Full:
            logger.warning("VR event queue is full; dropping event %s", event_type)
        except Exception as e:
            logger.warning("Failed to enqueue VR event %s: %s", event_type, e)

    def _process_button_events(self, hand: str, data: Dict):
        """Process A/B/thumbstick button edges regardless of grip state."""
        if not hand:
            return

        controller = self.left_controller if hand == "left" else self.right_controller

        button_a = data.get("buttonA", data.get("button_a"))
        button_b = data.get("buttonB", data.get("button_b"))
        thumbstick_pressed = data.get("thumbstickPressed", data.get("thumbstick_pressed"))
        pressed_buttons = data.get("pressedButtons", [])

        if self.event_queue is not None:
            if button_a is not None and bool(button_a) and not controller.button_a:
                logger.info(
                    "VR button A pressed (%s controller) payload buttons=%s", hand, pressed_buttons
                )
                self._emit_event("button_a", hand)
            if button_b is not None and bool(button_b) and not controller.button_b:
                logger.info(
                    "VR button B pressed (%s controller) payload buttons=%s", hand, pressed_buttons
                )
                self._emit_event("button_b", hand)
            if thumbstick_pressed is not None and bool(thumbstick_pressed) and not controller.thumbstick_pressed:
                logger.info(
                    "VR thumbstick pressed (%s controller) payload buttons=%s", hand, pressed_buttons
                )
                self._emit_event("thumbstick_press", hand)

        # Update latched button states (preserve previous if field missing)
        if button_a is not None:
            controller.button_a = bool(button_a)
        if button_b is not None:
            controller.button_b = bool(button_b)
        if thumbstick_pressed is not None:
            controller.thumbstick_pressed = bool(thumbstick_pressed)
    ...
    async def process_controller_data(self, data: Dict):
        ...
        if 'leftController' in data and 'rightController' in data:
            ...
            # Always process button events, even when grip isn't pressed.
            if isinstance(left_data, dict):
                self._process_button_events('left', left_data)
            if isinstance(right_data, dict):
                self._process_button_events('right', right_data)
        ...
        # Always process button events in legacy format too.
        if hand:
            self._process_button_events(hand, data)
    ...
    async def process_single_controller(self, hand: str, data: Dict):
        ...
        # Button events are handled in process_controller_data so they work even when grip isn't pressed.
