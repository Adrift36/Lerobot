...
AFRAME.registerComponent('controller-updater', {
  init: function () {
    ...
    // Button states
    // NOTE: Prefer A-Frame controller button events over navigator.getGamepads()
    // because WebXR gamepad mappings can vary across Quest / browsers.
    this._hasButtonEvents = false;
    this.leftButtonA = false;   // X on Quest
    this.leftButtonB = false;   // Y on Quest
    this.leftThumbstickPressed = false;
    this.rightButtonA = false;  // A on Quest
    this.rightButtonB = false;  // B on Quest
    this.rightThumbstickPressed = false;
    ...
    this.leftHand.addEventListener('gripup', (evt) => { // Add gripup listener
      ...
      // Quest: X/Y on left controller
      this.leftHand.addEventListener('xbuttondown', () => {
        console.log('Left X Pressed');
        this._hasButtonEvents = true;
        this.leftButtonA = true;
      });
      this.leftHand.addEventListener('xbuttonup', () => {
        console.log('Left X Released');
        this._hasButtonEvents = true;
        this.leftButtonA = false;
      });
      this.leftHand.addEventListener('ybuttondown', () => {
        console.log('Left Y Pressed');
        this._hasButtonEvents = true;
        this.leftButtonB = true;
      });
      this.leftHand.addEventListener('ybuttonup', () => {
        console.log('Left Y Released');
        this._hasButtonEvents = true;
        this.leftButtonB = false;
      });
      this.leftHand.addEventListener('thumbstickdown', () => {
        console.log('Left Thumbstick Pressed');
        this._hasButtonEvents = true;
        this.leftThumbstickPressed = true;
      });
      this.leftHand.addEventListener('thumbstickup', () => {
        console.log('Left Thumbstick Released');
        this._hasButtonEvents = true;
        this.leftThumbstickPressed = false;
      });
    ...
    this.rightHand.addEventListener('gripup', (evt) => { // Add gripup listener
      ...
      // Quest: A/B on right controller
      this.rightHand.addEventListener('abuttondown', () => {
        console.log('Right A Pressed');
        this._hasButtonEvents = true;
        this.rightButtonA = true;
      });
      this.rightHand.addEventListener('abuttonup', () => {
        console.log('Right A Released');
        this._hasButtonEvents = true;
        this.rightButtonA = false;
      });
      this.rightHand.addEventListener('bbuttondown', () => {
        console.log('Right B Pressed');
        this._hasButtonEvents = true;
        this.rightButtonB = true;
      });
      this.rightHand.addEventListener('bbuttonup', () => {
        console.log('Right B Released');
        this._hasButtonEvents = true;
        this.rightButtonB = false;
      });
      this.rightHand.addEventListener('thumbstickdown', () => {
        console.log('Right Thumbstick Pressed');
        this._hasButtonEvents = true;
        this.rightThumbstickPressed = true;
      });
      this.rightHand.addEventListener('thumbstickup', () => {
        console.log('Right Thumbstick Released');
        this._hasButtonEvents = true;
        this.rightThumbstickPressed = false;
      });
  ...
  getControllerButtonStates: function () {
    const states = {
      left: { buttonA: false, buttonB: false, thumbstickPressed: false },
      right: { buttonA: false, buttonB: false, thumbstickPressed: false }
    };
    ...
    for (const pad of pads) {
      if (!pad || !pad.hand) continue;

      const hand = pad.hand.toLowerCase();
      const target = hand === 'left' ? states.left : hand === 'right' ? states.right : null;
      if (!target) continue;

      // Oculus Touch (Quest) mapping is typically: 0 trigger, 1 grip, 2 A/X, 3 B/Y, 4 thumbstick press.
      // Some browsers expose extra indices; use a fallback scan.
      const buttons = pad.buttons || [];
      const pressedIdx = buttons.reduce((acc, btn, idx) => {
        if (btn && btn.pressed) acc.push(idx);
        return acc;
      }, []);

      // Heuristic mapping with fallbacks
      const bA = (buttons[2]?.pressed) || (buttons[4]?.pressed) || false;
      const bB = (buttons[3]?.pressed) || (buttons[5]?.pressed) || false;
      const thumb = (buttons[4]?.pressed) || (buttons[6]?.pressed) || false;

      target.buttonA = bA;
      target.buttonB = bB;
      target.thumbstickPressed = thumb;
      target.pressedButtons = pressedIdx;
    }
  ...
  tick: function () {
    ...
    // Refresh button states from WebXR gamepad inputs ONLY if we never got A-Frame button events.
    // This avoids overwriting event-driven states on platforms where gamepad.hand is missing.
    if (!this._hasButtonEvents) {
      const buttonStates = this.getControllerButtonStates();
      this.leftButtonA = buttonStates.left.buttonA;
      this.leftButtonB = buttonStates.left.buttonB;
      this.leftThumbstickPressed = buttonStates.left.thumbstickPressed;
      this.leftPressedButtons = buttonStates.left.pressedButtons;
      this.rightButtonA = buttonStates.right.buttonA;
      this.rightButtonB = buttonStates.right.buttonB;
      this.rightThumbstickPressed = buttonStates.right.thumbstickPressed;
      this.rightPressedButtons = buttonStates.right.pressedButtons;
    }
    ...
    if (this.leftHand.object3D.visible) {
        ...
        leftController.buttonA = this.leftButtonA;
        leftController.buttonB = this.leftButtonB;
        leftController.thumbstickPressed = this.leftThumbstickPressed;
        leftController.pressedButtons = this.leftPressedButtons || [];
    }
    ...
    if (this.rightHand.object3D.visible) {
        ...
        rightController.buttonA = this.rightButtonA;
        rightController.buttonB = this.rightButtonB;
        rightController.thumbstickPressed = this.rightThumbstickPressed;
        rightController.pressedButtons = this.rightPressedButtons || [];
    }
    ...
    if (this.websocket && this.websocket.readyState === WebSocket.OPEN) {
        ...
        if (hasValidLeft || hasValidRight) {
          ...
          // Debug one-shot when buttons are pressed
          if ((leftController.buttonA || leftController.buttonB || leftController.thumbstickPressed || leftController.pressedButtons.length > 0 || rightController.buttonA || rightController.buttonB || rightController.thumbstickPressed || rightController.pressedButtons.length > 0) && !this._buttonDebugLogged) {
            console.log("Sending button payload", dualControllerData);
            this._buttonDebugLogged = true;
            setTimeout(() => { this._buttonDebugLogged = false; }, 2000);
          }
